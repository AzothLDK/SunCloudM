...
class EleReportPage extends StatefulWidget {
  const EleReportPage({super.key});

  @override
  State<EleReportPage> createState() => _EleReportPageState();
}

class _EleReportPageState extends State<EleReportPage>
    with TickerProviderStateMixin {
  List<String> _tabs = [];
  List<Widget> _tabChildren = [];
  late TabController selController;
  ...
  getViewType() {
    if (seleteType == 1) {
      _tabs = [S.current.pv, S.current.ess];
      _tabChildren = [
        pvreportView(),
        storgeReportView(),
      ];
  ...
  Future<List> _getpvHourReport() async {
    ...
    var data = await ReportDao.getpvHourReport(params: params);
  ...
  Future<List> _getstorageHourReport() async {
    Map<String, dynamic> params = {};
    params['startTime'] = startDate;
    params['endTime'] = endDate;
    if (singleId != null) {
    ...
    print(params);
    var data = await ReportDao.getstorageHourReport(params: params);
    debugPrint(data.toString());
    if (data["code"] == 200) {
      if (data['data'] != null) {
      ...
      body: Container(
        ...
        child: Column(
          children: [
            ...
            Expanded(
              child:
                  TabBarView(controller: selController, children: getChilden()),
            ),
          ],
        ),
      ),
                  ...
                  SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      columns: [
                        ...
                        DataColumn(
                          label: Text('${S.current.generation}(kWh)'),
                        ),
                        DataColumn(
                          label: Text('${S.current.consume_energy}(kWh)'),
                        ),
                        DataColumn(
                          label: Text('${S.current.grid_energy}(kWh)'),
                        ),
                      ],
    ...
    if (datelist == null) {
      ...
      return datelist
          ...
          .map((e) => DataRow(
                ...
                cells: [
                  DataCell(Text(e.value["timeScale"] ?? "")),
                  DataCell(Text((e.value["generateEnergy"] ?? "").toString())),
                  DataCell(Text((e.value["consumeEnergy"] ?? "").toString())),
                  DataCell(Text((e.value["gridEnergy"] ?? "").toString())),
                ],
              ))
          .toList();
    }
                  ...
                  SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      columns: [
                        ...
                        DataColumn(
                          label: Text('${S.current.charging_amount}(kWh)'),
                        ),
                        DataColumn(
                          label: Text('${S.current.discharging_amount}(kWh)'),
                        ),
                      ],
    ...
    if (datelist == null) {
      ...
      return datelist
          ...
          .map((e) => DataRow(
                ...
                cells: [
                  DataCell(Text(e.value["timeScale"] ?? "")),
                  DataCell(Text((e.value["chargeAmount"] ?? "").toString())),
                  DataCell(Text((e.value["dischargeAmount"] ?? "").toString())),
                ],
              ))
          .toList();
    }
